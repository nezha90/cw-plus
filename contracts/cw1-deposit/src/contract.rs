...
use cosmwasm_std::{to_json_binary, Addr, Api, Binary, CosmosMsg, Deps, DepsMut,
...
use crate::query::{query_staking_info, query_all_users, query_contract_state};
use crate::user_update::update;
use crate::types::{CONTRACT_STATE, ContractState};
use crate::consts::{START_TIME, END_TIME};

// version info for migration info
const CONTRACT_NAME: &str = "crates.io:cw1-deposit";
...
pub fn instantiate(
    ...
    END_TIME.save(deps.storage, &msg.end_time)?;

    let contract_state = ContractState{
        total_staked: Uint128::zero(),
        total_rewards: Uint128::zero(),
        available_rewards: Uint128::zero()
    };
    CONTRACT_STATE.save(deps.storage, &contract_state)?;
...
pub fn execute(
    ...
    match msg {
        ...
        ExecuteMsg::Update { user } => update(deps, env, info, user),
        ExecuteMsg::ExtractFund { amount} => extract_fund(deps, env, info, amount),
    }
}
...
pub fn query(deps: Deps, _env: Env, msg: QueryMsg) -> StdResult<Binary> {
    match msg {
        ...
        QueryMsg::AllUsers {} =>  to_json_binary(&query_all_users(deps)?),
        QueryMsg::StakingInfo { user} => to_json_binary(&query_staking_info(deps, user)?),
        QueryMsg::ContractState {} => to_json_binary(&query_contract_state(deps)?),
    }
}
